...
### 含む
- プロジェクト雛形の整備
  - `package.json`（pnpm。scripts: `build` / `start` / `test` / `lint`）
  - `tsconfig.json`
  - `biome.json`（Lint/Format）
  - `vitest.config.ts`
  - `.env.example`（`WISHLIST_URL`, `DISCORD_WEBHOOK_URL`）
  - `pnpm-lock.yaml` をコミット（`pnpm install --frozen-lockfile` 用）
...
4. `pnpm start` で `src/main.ts` を実行できるようにする（`tsx` などのランナー、または `build` → `node dist`）。

## 完了条件（DoD 対応）

- [ ] クリーンな環境で `pnpm install --frozen-lockfile` のみで依存が揃う（**DoD 4.3 再現性**）。
- [ ] `WISHLIST_URL` / `DISCORD_WEBHOOK_URL` がコードにハードコードされず、`.env` / 環境変数から読まれる（**DoD 4.2 環境変数の秘匿**）。
- [ ] ローカル CLI から `pnpm start` を叩くと Discord にダミー通知が即時に届く（**DoD 4.1 実行の分離 / 手動実行**）。
- [ ] `pnpm lint` が通る。

## 検証方法

1. `.env` に自分の Discord Webhook URL を設定。
2. `pnpm install --frozen-lockfile && pnpm start` を実行。
3. Discord チャンネルにダミーメッセージが届くことを確認。
4. 別ディレクトリへ clone し直し、`pnpm install --frozen-lockfile` だけでビルド・起動できることを確認（再現性）。
