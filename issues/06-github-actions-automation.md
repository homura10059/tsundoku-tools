...
- `.github/workflows/monitor.yml`：
  ...
  - `pnpm install --frozen-lockfile` + `pnpm exec playwright install --with-deps` でブラウザバイナリ込みのセットアップ。
...
1. `.github/workflows/monitor.yml`：
   ...
   on:
     ...
     workflow_dispatch: {}
   ```
   - `actions/setup-node` + `pnpm/action-setup`（pnpm キャッシュ）→ `pnpm install --frozen-lockfile` → `pnpm exec playwright install --with-deps` → `pnpm start`。
   - `env:` に `WISHLIST_URL` / `DISCORD_WEBHOOK_URL` を `${{ secrets.* }}` から渡す。
2. トリガー部（ワークフローのスケジュール）と実行部（`pnpm start` の中身）が分離していることを担保。
...
- [ ] CI 環境で `pnpm install --frozen-lockfile` ＋ Playwright ブラウザインストールのみで過不足なくセットアップできる（**DoD 4.3 再現性**）。
...
3. ワークフローログで `pnpm install --frozen-lockfile` と `playwright install` が成功し、`pnpm start` が完走することを確認。
