...
describe("scrapeProduct", () => {
  it("extracts current price from .a-price-whole", async () => {
    ...
    const result = await scrapeProduct(TEST_ASIN, "https://example.com", noOpLimiter);
  ...
  it("throws when fetch returns a non-200 status", async () => {
    ...
    await expect(scrapeProduct(TEST_ASIN, "https://example.com", noOpLimiter)).rejects.toThrow(
